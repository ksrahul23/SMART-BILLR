...
export default function InvoiceGenerator({ onBack, initialData = null }) {
  ...
  // Load invoice data from initialData or localStorage on mount
  useEffect(() => {
    const data = initialData || (localStorage.getItem("currentInvoice") ? JSON.parse(localStorage.getItem("currentInvoice")) : null);
    ...
    try {
      ...
      if (session) {
        const invoiceData = {
          ...
          date: date ? date.toISOString() : null,
          dueDate: dueDate ? dueDate.toISOString() : null,
          notes, currency, items, logo, taxRate, discount,
          paymentMethod, bankName, accountNumber, accountName, routingNumber,
          terms, activeFields, pdfTheme
        };
        ...
        const { error: saveError } = await supabase.from("bills").insert([
          {
            ...
            currency: currency,
            invoice_date: date ? date.toISOString() : new Date().toISOString(),
            data: invoiceData
          }
        ]);
        ...
        if (saveError) {
          ...
          console.log("Bill saved successfully to history");
        }
      }
    } catch (error) {
